...
- 백엔드 개발자가 넘겨주는 양식은 다양하다
- js에 맞지 않는 양식이 많다 -> FE개발자가 알맞게 가공해야 한다
...
function solution(skill, skill_trees) {
  ...
  for (const skill_tree of skill_trees) {
    count += skill_tree
      ...
      .map((x) => skill.indexOf(x))
      .map((value, index) => (value === index ? true : false))
      .includes(false)
      ? 0
      : 1;
  }
...
function solution(skill, skill_trees) {
    ...
    for(const tree of skill_trees) {
        ...
        let myArr = tree.match(re)
        let idx = 0;
         while(myArr.length){
                if(skill[idx] != myArr[0]) break;
                idx++
                myArr.splice(0,1)
                if(myArr.length == 0) answer++;
        }
}
     return answer
}
